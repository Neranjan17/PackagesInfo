...
public class UpdateActivity extends AppCompatActivity {
	...
	private Button clearAllBtn, updatePackageBtn;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		updatePackageBtn = findViewById(R.id.updatePackage_btn);
	...
	private void updatePackage() {
		...
		int result = dbHelper.updatePackage(packageId, ispName, packageName, packagePrice, validDays, anyNetCalls,
				thisNetCalls, anyNetSms, thisNetSms, availableAppDataLimit, availableApp, paymentType, anyTimeData,
				dayTimeData, nightTimeData, description);

		if (result > 0) {
			Toast.makeText(this, "Package update successfully", Toast.LENGTH_SHORT).show();
	...
	private void lordPreviesData() {

		if (packageId >= 0) {

			Package pkg = dbHelper.getPackageById(packageId);

			ispNameAct.setText(pkg.getIspName());
			packageNameAct.setText(pkg.getPackageName());
			packagePriceAct.setText(pkg.getPackagePrice());
			validDaysAct.setText(pkg.getValidDays());
			anyNetCallsAct.setText(pkg.getAnyNetCalls());
			thisNetCallsAct.setText(pkg.getThisNetCalls());
			anyNetSmsAct.setText(pkg.getAnyNetSms());
			thisNetSmsAct.setText(pkg.getThisNetSms());
			availableAppDataLimitAct.setText(pkg.getAvailableAppsLimit());
			availableAppMact.setText(pkg.getAvailableApps());
			paymentTypeAct.setText(pkg.getDataPayType());
			anyTimeDataAct.setText(pkg.getAnyTimeData());
			dayTimeDataAct.setText(pkg.getDayTimeData());
			nightTimeDataAct.setText(pkg.getNightTimeData());
			descriptionMact.setText(pkg.getDescription());
	...
	private boolean validateInputs() {
		...
		// Check is duplicate
		if (dbHelper.isDuplicateUpdate(packageId, ispName, packagePrice)) {
